...
file open testfile using `output' , read
...
forvalues l=1/`servers' {
	local port`l' = ``l''
	cap file open f using "http://127.0.0.1:`port`l''/" , r
...
if `sum'==`servers' di as res "{col 25}running!"
	...
	forvalues l= 1/`servers' {
		if `server`l''==0 {
			if c(os)=="Windows" {
				qui file open bat using osrm_tempfiles/batch`l'.bat , write replace
				file write bat `"start /MIN /HIGH "OSRM" "`exefile'" "`mapfile'" -i 127.0.0.1 -p `port`l''"'
				file close bat
				winexec "`c(pwd)'/osrm_tempfiles/batch`l'.bat"
				}
			// else if c(os)=="Unix" {
			else {
				qui file open shell using osrm_tempfiles/osrm_serv`l'.sh , write replace
				file write shell `"`exefile' `mapfile' -i 127.0.0.1 -p `port`l''"'
				file close shell
...
if `parts'>1 {
	...
	while `l'<`parts' {
		...
		qui file open dofile using osrm_tempfiles/temp_instance`l'.do , write replace
		file write dofile "* do file generated by osrmtime" _n _n
		file write dofile `"global S_ADO = `"$S_ADO"'"' _n _n
		file write dofile `"adopath + "`c(pwd)'" "' _n
		file write dofile `"cd "`c(pwd)'/osrm_tempfiles""' _n
		file write dofile "capture noi{" _n
		file write dofile "use temp_package_routing`l'" _n _n
		file write dofile "osrminterface `varlist' , port(`port`portnum'') osrmver(`osrm_version')" _n
		...
		file write dofile "}" _n
		file write dofile "save temp_package_routed`l' , replace" _n _n
		file close dofile
		local ++l
		}
...
if c(os)=="Windows" {
    qui file open bat using osrm_tempfiles/__tmp_osrm.bat , write replace
    file write bat "pushd `c(pwd)'\osrm_tempfiles" _n
    file write bat "del temp_package_routed*" _n
    local l=1
    while `l'<`parts' {
    ...
    qui file open shell using osrm_tempfiles/__tmp_osrm.sh , write replace
    file write shell "cd `pwdir'/osrm_tempfiles" _n
    file write shell "rm temp_package_routed*" _n
    local l=1
    while `l'<`parts' {
        local tmpdir`l' = "`tmpdir'" + "_osrm12tmpdir00`l'"
        cap erase temp_package_routed`l'
    ...
    file open shell using osrm_tempfiles/__tmp_osrm.sh , write replace
    file write shell "cd `pwdir'/osrm_tempfiles" _n
    file write shell "rm temp_package_routed*" _n
    local l=1
    while `l'<`parts' {
        ...
        file write shell "mkdir -p `tmpdir`l''" _n
        file write shell "export STATATMP=`tmpdir`l''" _n
        file write shell `"`stata_executable' -e -q do temp_instance`l'.do &"' _n
        local ++l
    }
    file write shell "cd `pwdir'" _n
    file close shell 
